...
const packages = [
  {
    ...
    includes: ["Gypsy / Canter safari", "Forest naturalist guide", "Jungle resort stay", "All meals included"],
  ...
  },
  {
    ...
    desc: "Known as the Tiger Capital of India, Bandhavgarh boasts the highest tiger density in the world, set within ancient sal forests and dramatic cliffs.",
    highlights: ["Highest tiger density in India", "Ancient Bandhavgarh Fort", "White tiger legacy zone", "Morning & evening safaris"],
    includes: ["Gypsy / Canter safari", "Expert naturalist guide", "Forest lodge stay", "Safari breakfast included"],
    animals: ["Bengal Tiger", "Leopard", "Sloth Bear", "Wild Dog", "Gaur"],
    bestTime: "Oct – June",
  ...
  },
  {
    ...
    desc: "A UNESCO Biosphere Reserve along the banks of the Ken River, Panna is famed for its tiger comeback story, stunning gorges and rich diamond heritage.",
    highlights: ["Ken River boat safari", "Tiger & vulture sightings", "Ken Gharial Sanctuary", "Pandav Falls visit"],
    includes: ["Gypsy / Canter safari", "Certified naturalist guide", "Riverside camp stay", "All meals included"],
    animals: ["Tiger", "Gharial", "Vulture", "Chital", "Sloth Bear"],
    bestTime: "Oct – June",
    IconA: Sun, IconB: Waves,
  },
];

const activities = [
  { icon: Binoculars, title: "Jeep Safari Rides", desc: "Roar through dense jungle corridors in open-top gypsies — the classic wildlife safari experience with expert naturalist guides." },
  { icon: Camera, title: "Wildlife Photography", desc: "Capture Bengal tigers, leopards, gharials and rare bird species in their natural splendour across three incredible reserves." },
  { icon: Tent, title: "Luxury Jungle Camps", desc: "Spend nights in candlelit tented camps and forest lodges deep inside the jungle, steps away from the wild." },
  { icon: Flame, title: "Sundowners & Bonfires", desc: "Gather around a campfire as the jungle comes alive at dusk, with folk artists and naturalists sharing stories of the wild." },
];
...
const places = [
  ...
  },
  {
    ...
    info: "Highest tiger density in the world. Ancient fort, dense sal forests and legendary sightings await in Madhya Pradesh.",
  },
  {
    name: "Panna National Park",
    ...
    info: "A stunning Ken River valley reserve famous for tigers, gharials, vultures and the remarkable tiger reintroduction story.",
  },
];
...
export default function App() {
  ...
  const [vehicleType, setVehicleType] = useState("gypsy");
  ...
  const pricePerPerson = pkg ? (vehicleType === "canter" ? pkg.canterPrice : pkg.gypsyPrice) : 0;
  ...
  return (
    <div style={{ fontFamily:"'Playfair Display',serif", background:"#c9d4cb", minHeight:"100vh", color:"#1a3d2b" }}>
      <style>{`
        ...
        .hero-section{position:relative;min-height:100svh;display:flex;align-items:flex-end;overflow:hidden;}
        .hero-bg{position:absolute;inset:0;background-image:url('https://i.pinimg.com/736x/36/3f/b8/363fb80cae08f49bf7612664f2f2c246.jpg');background-size:cover;background-position:center 40%;animation:kenBurns 18s ease-in-out infinite alternate;}
        @keyframes kenBurns{from{transform:scale(1) translate(0,0);}to{transform:scale(1.06) translate(-1%,-1%);}}
        .hero-overlay{position:absolute;inset:0;background:linear-gradient(to top,rgba(0,0,0,0.95) 0%,rgba(0,0,0,0.75) 30%,rgba(0,0,0,0.45) 60%,rgba(0,0,0,0.20) 85%,rgba(0,0,0,0.05) 100%);}
        .hero-content{position:relative;z-index:2;width:100%;padding:120px 24px 56px;max-width:1200px;margin:0 auto;}
        @media(max-width:768px){.hero-content{padding:100px 20px 48px;}}
        @media(max-width:480px){.hero-content{padding:80px 16px 40px;}}

        .hero-headline{font-size:clamp(38px,7vw,82px);font-weight:800;color:#fff;line-height:1.0;letter-spacing:-1px;margin-bottom:20px;}
        @media(max-width:480px){.hero-headline{font-size:clamp(28px,6vw,48px);margin-bottom:16px;}}
        .hero-headline em{font-style:italic;font-weight:400;color:#a8d4b0;}
        .hero-sub{color:rgba(200,230,210,0.85);font-size:clamp(14px,2vw,17px);line-height:1.8;max-width:520px;margin-bottom:32px;font-weight:400;}
        @media(max-width:480px){.hero-sub{font-size:clamp(13px,2vw,15px);margin-bottom:24px;line-height:1.6;}}

        .hero-btns{display:flex;gap:12px;flex-wrap:wrap;margin-bottom:52px;}
        @media(max-width:480px){.hero-btns{margin-bottom:40px;gap:10px;}}
        .btn-white{background:#fff;color:#1a3d2b;border:none;padding:14px 28px;border-radius:50px;font-size:14px;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:8px;transition:all .2s;font-family:'DM Sans',sans-serif;white-space:nowrap;}
        .btn-white:hover{background:#e8f5ec;transform:translateY(-2px);}
        .btn-outline{background:transparent;color:#fff;border:1.5px solid rgba(255,255,255,0.40);padding:14px 28px;border-radius:50px;font-size:14px;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:8px;transition:all .2s;font-family:'DM Sans',sans-serif;white-space:nowrap;}
        .btn-outline:hover{background:rgba(255,255,255,0.10);border-color:rgba(255,255,255,0.65);}

        .hero-stats{display:flex;gap:0;border-top:1px solid rgba(255,255,255,0.14);padding-top:28px;flex-wrap:wrap;}
        @media(max-width:640px){.hero-stats{padding-top:20px;}}
        .hero-stat{flex:1;min-width:120px;padding:0 28px 0 0;border-right:1px solid rgba(255,255,255,0.12);margin-right:28px;}
        ...
        .vehicle-btn{cursor:pointer;transition:all .2s;font-family:'DM Sans',sans-serif;border:none;}
        ...
        @media(max-width:640px){.pkg-inner{grid-template-columns:1fr;grid-template-rows:200px auto}}
        ...
        @media(max-width:900px){.left-content{position:static}}

        .step-tab{transition:opacity .2s}
        .step-tab.go{cursor:pointer}
        .step-nav-label{display:inline}
        @media(max-width:480px){.step-nav-label{display:none}}
        ...
        @media(max-width:480px){.act-grid{grid-template-columns:1fr 1fr}}
        ...
        @media(max-width:520px){.hero-stat{min-width:100px;padding:0 16px 0 0;margin-right:16px;}.hero-stat-val{font-size:22px}}
        @media(max-width:360px){.hero-stats{gap:16px}.hero-stat{border-right:none;margin-right:0;padding:0}.hero-stat:not(:last-child){padding-bottom:12px;border-bottom:1px solid rgba(255,255,255,0.1)}}
        ...
        @media(max-width:520px){.success-card{padding:48px 24px}}

        .sec-pad{padding:60px 24px}
        @media(max-width:640px){.sec-pad{padding:40px 16px}}
        .sec-pad-lg{padding:60px 24px 80px}
        @media(max-width:640px){.sec-pad-lg{padding:40px 16px 60px}}

        .pkg-content-pad{padding:22px 24px 20px}
        @media(max-width:640px){.pkg-content-pad{padding:16px 18px 18px}}
      `}</style>
      ...
      <header className="hero-section">
        ...
        <div className="hero-content">
          ...
          <p className="hero-sub">
            Roar alongside Bengal tigers in Ranthambore, discover the world's densest tiger population in Bandhavgarh, and witness the remarkable wildlife comeback story of Panna — three legendary reserves, one unforgettable journey.
          </p>
      
          <div className="hero-stats">
            {[["3","Tiger Reserves"],["4.8 / 5","Average Rating"],["8,200+","Guests Hosted"]].map(([val,lbl])=>(
      ...
      <div style={{ background:"#1a3d2b", padding:"18px 24px", overflowX:"auto" }}>
        <div style={{ maxWidth:1160, margin:"0 auto", display:"flex", alignItems:"center", gap:32, whiteSpace:"nowrap", justifyContent:"center", flexWrap:"wrap" }}>
          {[
            ...
            [Bird,"Bird Watching"],
            [Mountain,"Tiger Tracking"],
            [Waves,"River Safari"],
          ].map(([Icon,lbl])=>(
            <div key={lbl} style={{ display:"flex", alignItems:"center", gap:8, color:"#a8c4ae", fontSize:13, fontWeight:600 }}>
      ...
      <section style={{ background:"#b8c9bc" }} className="sec-pad">
        <div style={{ maxWidth:1160, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:36 }}>
            <p style={{ color:"#4a7a5c", fontSize:12, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", marginBottom:8 }}>What You'll Experience</p>
            <h2 style={{ fontSize:"clamp(24px,4vw,36px)", fontWeight:800, color:"#1a3d2b" }}>The Joy of Tiger Safari</h2>
          </div>
          <div className="act-grid">
            {activities.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="act-card">
                ...
                <div style={{ fontWeight:700, fontSize:15, color:"#1a3d2b", marginBottom:8 }}>{title}</div>
                <div style={{ color:"#4a7a5c", fontSize:13, lineHeight:1.65 }}>{desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="places" className="sec-pad" style={{ background:"#c9d4cb" }}>
        <div style={{ maxWidth:1160, margin:"0 auto" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:36, flexWrap:"wrap", gap:16 }}>
            <div>
              <p style={{ color:"#4a7a5c", fontSize:12, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", marginBottom:8 }}>Destinations</p>
              <h2 style={{ fontSize:"clamp(24px,4vw,36px)", fontWeight:800, color:"#1a3d2b" }}>The Three Tiger Reserves</h2>
            </div>
            <p style={{ color:"#4a7a5c", fontSize:14, maxWidth:380, lineHeight:1.65 }}>Each reserve offers a completely unique ecosystem — from Rajasthan's royal landscapes to the dense forests of Madhya Pradesh.</p>
          </div>
            ...
            {places.map(p => (
              <div key={p.name} className="place-card">
                ...
                <div style={{ padding:"18px 20px 20px" }}>
                  ...
                  <div style={{ color:"#4a7a5c", fontSize:13, lineHeight:1.65 }}>{p.info}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="packages" className="sec-pad-lg" style={{ background:"#b8c9bc" }}>
        <div style={{ maxWidth:1160, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:40 }}>
            <p style={{ color:"#4a7a5c", fontSize:12, fontWeight:700, letterSpacing:".12em", textTransform:"uppercase", marginBottom:8 }}>Book Your Adventure</p>
          ...
          <div style={{ background:"#a8bcad", borderRadius:14, padding:"0 8px", marginBottom:32, display:"inline-flex", width:"100%", position:"sticky", top:0, zIndex:10 }}>
            {[{n:1,lbl:"Choose Park"},{n:2,lbl:"Date & Guests"},{n:3,lbl:"Your Details"}].map(({n,lbl},i)=>(
              <div key={n} className={`step-tab${step>=n?" go":""}`} onClick={()=>step>=n&&setStep(n)}
                ...
                <span className="step-nav-label" style={{ fontWeight:step===n?700:500, fontSize:13, color:"#1a3d2b", whiteSpace:"nowrap" }}>{lbl}</span>
          ...
          {done ? (
            <div className="success-card">
              ...
              <p style={{ color:"#4a7a5c", fontSize:16, marginBottom:4 }}>Thank you, <strong>{form.name}</strong>. Your jungle adventure awaits!</p>
              <p style={{ color:"#8aad93", fontSize:14, marginBottom:36 }}>Confirmation sent to <strong>{form.email}</strong></p>
              <div style={{ display:"flex", gap:20, background:"#f6f9f7", borderRadius:14, padding:"20px 24px", border:"1px solid #c9d4cb", marginBottom:40, flexWrap:"wrap", justifyContent:"center" }}>
                {[[pkg?.name,"Park"],[`${month} 2026`,"Departure"],[vehicleType==="canter"?"Canter":"Gypsy","Vehicle"],[`${guests} guest${guests>1?"s":""}`,"Guests"],[`₹${final.toLocaleString()}`,"Total"]].map(([v,l])=>(
                  <div key={l} style={{ textAlign:"center", minWidth:80 }}>
                    <div style={{ fontWeight:800, fontSize:14, color:"#1a3d2b" }}>{v}</div>
                    <div style={{ fontSize:10, color:"#8aad93", fontWeight:600, letterSpacing:".06em", marginTop:3 }}>{l.toUpperCase()}</div>
                  </div>
                ))}
              </div>
              <button className="btn-primary" onClick={()=>{setDone(false);setStep(1);setSel(null);setVehicleType("gypsy");setForm({name:"",email:"",phone:"",note:""});}}
              ...
              <div className="left-content">
                          ...
                          <div className="pkg-inner">
                            <div style={{ position:"relative", overflow:"hidden", minHeight:200 }}>
                              ...
                              <div style={{ position:"absolute", top:14, left:14, display:"flex", alignItems:"center", gap:5, background:p.accentLight, borderRadius:20, padding:"5px 12px" }}>
                                ...
                                <span style={{ color:p.accentDark, fontSize:11, fontWeight:700, letterSpacing:".07em" }}>{p.badge.toUpperCase()}</span>
                              </div>
                              <div style={{ position:"absolute", bottom:0, left:0, right:0, padding:"16px 18px" }}>
                                ...
                                <div style={{ display:"flex", alignItems:"center", gap:5, color:"rgba(255,255,255,.75)", fontSize:12 }}>
                              ...
                              <div>
                                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8, marginBottom:12 }}>
                                  <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
                                    <span style={{ display:"flex", alignItems:"center", gap:5, color:"#4a7a5c", fontSize:13 }}>
                                      <Clock size={13} color="#6b9478"/>{p.duration}
                                    </span>
                                    <span style={{ display:"flex", alignItems:"center", gap:5, fontSize:13 }}>
                                      <Stars rating={p.rating}/><b style={{ color:"#1a3d2b" }}>{p.rating}</b><span style={{ color:"#8aad93" }}>({p.reviews})</span>
                                    </span>
                                  </div>
                                ...
                                <div style={{ marginBottom:14 }}>
                                  <div style={{ fontSize:11, fontWeight:700, color:"#8aad93", letterSpacing:".08em", marginBottom:7 }}>WILDLIFE YOU MAY SPOT</div>
                                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                                    {p.animals.map(a=>(
                                      <span key={a} style={{ background:"#f0f7f2", color:"#2d6644", fontSize:11, fontWeight:600, padding:"4px 10px", borderRadius:20, border:"1px solid #c9d4cb" }}>{a}</span>
                                    ))}
                                  </div>
                                </div>
                                ...
                                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"6px 12px" }}>
                                  {p.includes.map(inc=>(
                                    <div key={inc} style={{ display:"flex", alignItems:"flex-start", gap:6 }}>
                                      ...
                                      <span style={{ color:"#2d6644", fontSize:12, fontWeight:500, lineHeight:1.4 }}>{inc}</span>
                                    </div>
                                  ))}
                                </div>
                              </div>

                              {/* Pricing with vehicle toggle */}
                              <div style={{ marginTop:18, paddingTop:16, borderTop:"1px solid #edf2ee" }}>
                                <div style={{ fontSize:11, fontWeight:700, color:"#8aad93", letterSpacing:".08em", marginBottom:10 }}>SAFARI VEHICLE & PRICE</div>
                                <div style={{ display:"flex", gap:10, marginBottom:12, flexWrap:"wrap" }}>
                                  {[["canter","Canter",p.canterPrice],["gypsy","Gypsy",p.gypsyPrice]].map(([v,lbl,price])=>{
                                    const isOn = on && vehicleType===v;
                                    return (
                                      <button key={v} className="vehicle-btn"
                                        onClick={e=>{e.stopPropagation();setSel(p.id);setVehicleType(v);}}
                                        style={{ flex:1, padding:"10px 14px", borderRadius:12, background:isOn?"#1a3d2b":"#f0f7f2", color:isOn?"#fff":"#1a3d2b", border:`2px solid ${isOn?"#1a3d2b":"#c9d4cb"}`, textAlign:"left" }}>
                                        <div style={{ fontSize:13, fontWeight:700 }}>{lbl}</div>
                                        <div style={{ fontSize:18, fontWeight:800, fontFamily:"'Playfair Display',serif", marginTop:2 }}>₹{price.toLocaleString()}</div>
                                        <div style={{ fontSize:11, opacity:.7, marginTop:1 }}>per person</div>
                                      </button>
                                    );
                                  })}
                                </div>
                                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:10 }}>
                                  <div style={{ color:"#6b9478", fontSize:11 }}>Best time: {p.bestTime}</div>
                                  <div style={{ display:"flex", alignItems:"center", gap:6, background:on?"#1a3d2b":"#f0f7f2", color:on?"#fff":"#1a3d2b", padding:"9px 18px", borderRadius:10, fontSize:13, fontWeight:700, transition:"all .2s", flexShrink:0 }}>
                ...
                {step===2&&(
                  <div>
                    ...
                    <p style={{ color:"#4a7a5c", fontSize:15, marginBottom:24 }}>Pick your preferred travel month and group size.</p>
                    ...
                    <div style={{ background:"#fff", borderRadius:18, padding:"26px 20px", marginBottom:18, boxShadow:"0 2px 14px rgba(26,61,43,.06)" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:18, flexWrap:"wrap" }}>
                        ...
                        <span style={{ fontWeight:700, fontSize:16 }}>Travel Month</span>
                        <span style={{ marginLeft:"auto", background:"#f0f7f2", color:"#2d6644", fontSize:12, fontWeight:700, padding:"4px 12px", borderRadius:20, border:"1px solid #c9d4cb" }}>{month} 2026</span>
                      </div>
                    ...
                    <div style={{ background:"#fff", borderRadius:18, padding:"26px 20px", marginBottom:18, boxShadow:"0 2px 14px rgba(26,61,43,.06)" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:22 }}>
                        ...
                        <span style={{ fontWeight:700, fontSize:16 }}>Number of Guests</span>
                      </div>
                      <div style={{ display:"flex", alignItems:"center", gap:16, flexWrap:"wrap" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:14, background:"#f4f7f5", borderRadius:14, padding:"8px 12px" }}>
                          ...
                          <div style={{ textAlign:"center", minWidth:44 }}>
                            ...
                            <div style={{ fontSize:10, color:"#8aad93", fontWeight:700, letterSpacing:".06em", marginTop:2 }}>GUESTS</div>
                          </div>
                        ...
                        {discount?(
                          <div style={{ display:"flex", alignItems:"center", gap:8, background:"#f0f7f2", borderRadius:10, padding:"10px 14px", border:"1px solid #c9d4cb" }}>
                            ...
                            <div>
                              <div style={{ color:"#1a3d2b", fontSize:13, fontWeight:700 }}>10% Group Discount Applied!</div>
                              <div style={{ color:"#6b9478", fontSize:12 }}>Saving ₹{Math.round(total*.1).toLocaleString()}</div>
                            </div>
                          </div>
                        ):(
                          <div style={{ color:"#6b9478", fontSize:13 }}><span style={{ color:"#1a3d2b", fontWeight:700 }}>Groups of 6+</span> receive 10% off</div>
                        )}
                      </div>
                    </div>
                ...
                {step===3&&(
                  <div>
                    ...
                    <p style={{ color:"#4a7a5c", fontSize:15, marginBottom:24 }}>We'll send your booking confirmation and itinerary here.</p>
                    ...
                    <div style={{ background:"#fff", borderRadius:18, padding:"24px 20px", boxShadow:"0 2px 14px rgba(26,61,43,.06)", marginBottom:18 }}>
                      <div className="form-grid">
                        {[
                          ...
                          <div key={k}>
                            <label style={{ display:"flex", alignItems:"center", gap:6, fontWeight:600, fontSize:12, color:"#1a3d2b", marginBottom:8, letterSpacing:".05em", textTransform:"uppercase" }}>
                ...
                <div style={{ background:"#fff", borderRadius:20, overflow:"hidden", boxShadow:"0 6px 28px rgba(26,61,43,.1)" }}>
                  <div style={{ background:"#1a3d2b", padding:"22px 24px 20px", position:"relative", overflow:"hidden" }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:8 }}>
                      ...
                      <span style={{ color:"#6b9478", fontSize:11, fontWeight:700, letterSpacing:".1em" }}>BOOKING SUMMARY</span>
                    </div>
                    {pkg?(
                      <>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:5, color:"#8aad93", fontSize:12, flexWrap:"wrap" }}>
                      ...
                      <div style={{ color:"#a8c4ae", fontSize:15, fontWeight:600 }}>No park selected</div>
                    )}
                  </div>
                      ...
                      <>
                        <div style={{ display:"flex", flexDirection:"column", gap:11, marginBottom:16 }}>
                          {[
                            ...
                            [Binoculars,"Vehicle",vehicleType==="canter"?"Canter":"Gypsy"],
                            [Star,"Per Person",`₹${pricePerPerson.toLocaleString()}`],
                            ...(discount?[[BadgeCheck,"Group Discount","−10%"]]:[]),
                          ].map(([Icon,k,v])=>(
                            <div key={k} style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                              <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                                <Icon size={13} color="#8aad93"/>
                                <span style={{ color:"#6b9478", fontSize:13, fontWeight:500 }}>{k}</span>
                              </div>
                              <span style={{ color:"#1a3d2b", fontSize:13, fontWeight:700 }}>{v}</span>
                            </div>
                          ))}
                        </div>
                        <div style={{ borderTop:"1.5px dashed #e2e8e4", paddingTop:14, marginBottom:16 }}>
                          ...
                          <div style={{ color:"#8aad93", fontSize:11, textAlign:"right", marginTop:2 }}>All taxes & fees included</div>
                        </div>
                      </>
                    ):(
                      <div style={{ textAlign:"center", padding:"20px 0 8px" }}>
                        <Globe size={36} color="#c9d4cb" style={{ margin:"0 auto 10px", display:"block" }}/>
                        <div style={{ fontSize:14, fontWeight:500, color:"#8aad93" }}>Select a park to see pricing</div>
                      </div>
                    )}
                    <div style={{ background:"#f6f9f7", borderRadius:12, padding:"14px 16px", border:"1px solid #e4edea" }}>
                      {[
                        ...
                        <div key={txt} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:i<2?10:0 }}>
                          ...
                          <span style={{ color:"#2d6644", fontSize:12, fontWeight:500 }}>{txt}</span>
                        </div>
                      ))}
                    </div>
                  </div>
