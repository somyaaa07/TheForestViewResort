...
const IMGS = {
  ...
  team:     "https://images.unsplash.com/photo-1540553016722-983e48a2cd10?w=600&q=80",
};

// Amritsar office coordinates
...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    body { background: ${G.bg}; font-family: 'Jost', sans-serif; color: ${G.ink}; -webkit-font-smoothing: antialiased; overflow-x: hidden; margin:0; padding:0; }
    ...
    .fi { width:100%; background:transparent; border:none; border-bottom:1.5px solid rgba(4,17,6,.18); padding:11px 0 9px; font-family:'Jost',sans-serif; font-size:15px; font-weight:300; color:${G.ink}; outline:none; transition:border-color .25s; border-radius:0; }
    .fi::placeholder { color:rgba(4,17,6,.3); }
    .fi:focus { border-bottom-color:${G.ink}; }
    textarea.fi { resize:none; min-height:88px; line-height:1.7; }
    ...
    .chip { padding:6px 14px; border:1px solid rgba(4,17,6,.14); background:transparent; font-family:'Jost',sans-serif; font-size:12px; font-weight:400; color:rgba(4,17,6,.55); cursor:pointer; transition:all .2s; letter-spacing:.03em; border-radius:999px; }
    .chip:hover { color:${G.ink}; border-color:rgba(4,17,6,.4); }
    .chip.on { background:${G.ink}; color:${G.bg}; border-color:${G.ink}; }

    .btn-p { position:relative; overflow:hidden; display:inline-flex; align-items:center; gap:10px; background:${G.ink}; color:${G.bg}; border:none; padding:14px 28px; font-family:'Jost',sans-serif; font-size:12px; font-weight:500; letter-spacing:.1em; text-transform:uppercase; cursor:pointer; transition:transform .22s, box-shadow .22s; white-space:nowrap; }
    .btn-p:hover { transform:translateY(-2px); box-shadow:0 12px 36px rgba(4,17,6,.22); }
    .btn-p:active { transform:scale(.97); }
    .btn-p:disabled { opacity:.5; cursor:not-allowed; transform:none; }

    .btn-g { display:inline-flex; align-items:center; gap:8px; background:transparent; border:1px solid rgba(4,17,6,.22); padding:10px 20px; font-family:'Jost',sans-serif; font-size:12px; font-weight:500; letter-spacing:.08em; text-transform:uppercase; color:${G.ink}; cursor:pointer; transition:all .22s; text-decoration:none; white-space:nowrap; }
    .btn-g:hover { background:${G.ink}; color:${G.bg}; }

    .itile { display:flex; align-items:flex-start; gap:16px; padding:18px 20px; border:1px solid rgba(4,17,6,.09); background:rgba(255,255,255,.5); transition:all .25s; }
    ...
    .faq-btn { width:100%; background:none; border:none; cursor:pointer; display:flex; align-items:center; justify-content:space-between; padding:18px 0; gap:16px; text-align:left; font-family:'Jost',sans-serif; font-size:14px; font-weight:400; color:${G.ink}; }
    ...
    .dest { position:relative; overflow:hidden; cursor:pointer; }
    ...
    .map-iframe-wrap iframe {
      ...
      border: none;
      display: block;
      filter: sepia(20%) contrast(95%) brightness(102%);
    }
    /* Custom overlay pin that matches brand */
    .map-pin-overlay {
      position: absolute;
      top: 50%; left: 50%;
    ...
    .map-pin-head {
      ...
      box-shadow: 0 6px 24px rgba(4,17,6,.4);
    }
    .map-pin-tail {
      width: 0; height: 0;
      border-left: 8px solid transparent;
      border-right: 8px solid transparent;
      border-top: 10px solid ${G.ink};
    }
    ...
    @media (max-width: 900px) {
      ...
      .hero-content { padding:0 28px 64px !important; }
      .hero-title { font-size:clamp(3rem,10vw,5.5rem) !important; }
      .hero-stats { gap:28px !important; }
      .section-pad { padding:60px 0 0 !important; }
      .section-pad-both { padding:60px 0 !important; }
      .dest-strip { grid-template-columns:1fr 1fr !important; gap:3px !important; }
      .faq-img { height:200px !important; }
      .faq-text-block, .faq-btn-block { display:none; }
      ...
      .map-strip { flex-direction:column !important; }
      .map-strip-item { border-right:none !important; border-bottom:1px solid rgba(4,17,6,.1) !important; }
      .map-strip-item:last-child { border-bottom:none !important; }
    }
    @media (max-width: 640px) {
      ...
      .grid-4 { grid-template-columns:1fr 1fr; }
      .grid-contact, .grid-faq { grid-template-columns:1fr; }
      ...
      .section-pad { padding:44px 0 0 !important; }
      .section-pad-both { padding:44px 0 56px !important; }
      .dest-strip { grid-template-columns:1fr !important; }
      .dest-item { height:220px !important; }
      .contact-form { padding:28px 20px !important; }
      .form-chips { gap:6px !important; }
      ...
      .map-iframe-wrap iframe { height:280px !important; }
      .map-badge { display:none !important; }
      .faq-img { height:160px !important; }
      .faq-section { padding:44px 0 56px !important; }
      .faq-q { font-size:13px !important; }
      .footer-pad { padding:36px 18px 24px !important; }
      .footer-bottom { flex-direction:column !important; gap:8px !important; text-align:center !important; }
      .footer-brand { grid-column:1/-1 !important; }
      .dest-label p:last-child { font-size:18px !important; }
      .section-heading { font-size:1.3rem !important; }
      .subsection-row { flex-direction:column !important; align-items:flex-start !important; gap:12px !important; }
      .map-head-row { flex-direction:column !important; align-items:flex-start !important; gap:8px !important; }
    }
...
function Hero() {
  ...
  return (
    <section style={{ position:"relative", minHeight:"100vh", display:"flex", flexDirection:"column", justifyContent:"flex-end", overflow:"hidden" }}>
      ...
      <div className="hero-content" style={{ position:"relative", maxWidth:1280, margin:"0 auto", padding:"0 48px 90px", width:"100%" }}>
        ...
        <div ref={r3} className="hero-stats" style={{ display:"flex", gap:40, marginTop:48, flexWrap:"wrap" }}>
...
function DestStrip() {
  ...
  return (
    <section className="section-pad" style={{ padding:"72px 0 0" }}>
      <div className="container">
        <div ref={ref} className="subsection-row" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24, gap:12, flexWrap:"wrap" }}>
      ...
      <div className="dest-strip grid-4" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:3 }}>
        {dests.map((d) => (
...
function ContactForm() {
  ...
  if (status === "success") return (
    <div ref={ref} className="contact-form" style={{ padding:"64px 48px", background:"rgba(255,255,255,.6)", border:`1px solid ${G.border}`, display:"flex", flexDirection:"column", alignItems:"center", gap:16, textAlign:"center", animation:"fadeUp .5s ease" }}>
  ...
  return (
    <div id="contact" ref={ref} className="contact-form" style={{ background:"rgba(255,255,255,.62)", border:`1px solid ${G.border}`, padding:"40px 40px" }}>
      <div className="form-header" style={{ display:"flex", gap:0, marginBottom:28, paddingBottom:24, borderBottom:`1px solid ${G.border}`, alignItems:"flex-start", justifyContent:"space-between" }}>
        ...
        <div style={{ display:"flex", gap:4, alignItems:"center", flexShrink:0 }}>
          {[IMGS.mountains, IMGS.beach, IMGS.forest].map((img, i) => (
            <div key={i} style={{ width:40, height:40, borderRadius:"50%", overflow:"hidden", border:"2px solid rgba(255,255,255,.8)", marginLeft: i > 0 ? -10 : 0 }}>
      ...
      <form onSubmit={submit} style={{ display:"flex", flexDirection:"column", gap:22 }}>
        ...
        <div>
          ...
          <select className="fi" value={form.budget} onChange={set("budget")}>
            ...
            <option>₹50,000 – ₹1,00,000</option><option>₹1,00,000 – ₹2,00,000</option><option>₹2,00,000+</option>
          </select>
        </div>
        <div><label className="fl">Your Message *</label><textarea className="fi" placeholder="Tell us about your dream trip — travel dates, group size, special requirements…" value={form.message} onChange={set("message")} required /></div>
...
const INFO = [
  ...
  { icon:"✉",  label:"Email Us",           lines:["hello@THE FOREST VIEW.travel","support@THE FOREST VIEW.travel"],           cta:"Send Email →"     },
];
...
function ContactInfo() {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
      <div className="info-image" style={{ position:"relative", height:210, overflow:"hidden", marginBottom:24 }}>
        ...
        {INFO.map((item, i) => (
          <div ref={refs[i]} key={item.label} className="itile">
            ...
            <div style={{ flex:1, minWidth:0 }}>
              ...
              {item.lines.map(l => <p key={l} style={{ fontSize:"14px", fontWeight:300, color:G.ink, lineHeight:1.55, wordBreak:"break-word", margin:0 }}>{l}</p>)}
...
function WhyUs() {
  ...
  return (
    <section className="section-pad" style={{ maxWidth:1280, margin:"0 auto", padding:"80px 48px 0" }}>
      <div ref={ref} style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", marginBottom:30, flexWrap:"wrap", gap:16 }}>
        <div>
      ...
      <div className="why-cards grid-3" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12 }}>
        {feats.map((f, i) => (
          <div key={f.title} style={{ position:"relative", overflow:"hidden", cursor:"pointer" }}
...
/* ─── MAP SECTION — iframe embed (always works) ─── */
function MapSection() {
  ...
  return (
    <section className="section-pad" style={{ maxWidth:1280, margin:"0 auto", padding:"80px 48px 0" }}>
      {/* Header */}
      <div ref={headRef} className="map-head-row" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24, gap:12, flexWrap:"wrap" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          <div style={{ width:28, height:1, background:G.ink, opacity:.25 }} />
        ...
        <div style={{ display:"flex", alignItems:"center", gap:7 }}>
          ...
          <span style={{ fontSize:"11px", color:"rgba(4,17,6,.48)", fontWeight:300, letterSpacing:".02em" }}>
      ...
      <div className="map-iframe-wrap">
        ...
        {/* Brand badge overlay — floats above iframe */}
      ...
      {/* Info strip under map */}
      <div className="map-strip" style={{
        ...
        {[
          ...
          { icon:"✉",  label:"Email",        val:"hello@THE FOREST VIEW.travel" },
        ].map((item, i) => (
          <div key={i} className="map-strip-item" style={{
            ...
            <div>
              ...
              <p style={{ fontSize:"13px", fontWeight:300, color:G.ink, margin:0 }}>{item.val}</p>
            </div>
          </div>
        ))}
      </div>
...
function FAQ() {
  ...
  const ref = useReveal(80);
  return (
    <section className="faq-section section-pad-both" style={{ maxWidth:1280, margin:"0 auto", padding:"80px 48px 88px" }}>
      <div ref={ref} className="grid-faq" style={{ display:"grid", gridTemplateColumns:"1fr 1.8fr", gap:80, alignItems:"start" }}>
        <div>
          <div className="faq-img" style={{ position:"relative", height:240, overflow:"hidden", marginBottom:24 }}>
...
function Footer() {
  return (
    <footer style={{ borderTop:`1px solid ${G.border}`, background:"rgba(4,17,6,.03)" }}>
      <div className="footer-pad" style={{ maxWidth:1280, margin:"0 auto", padding:"44px 48px 32px" }}>
        <div className="grid-footer" style={{ display:"grid", gridTemplateColumns:"1fr auto auto auto", gap:40, marginBottom:40 }}>
          <div className="footer-brand" style={{ maxWidth:240 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:12 }}>
              <div style={{ width:30, height:30, background:G.ink, display:"flex", alignItems:"center", justifyContent:"center" }}>
          ...
          {[["Company",["About","Careers","Press","Blog"]],["Services",["Packages","Custom Tours","Group Travel","Corporate"]],["Support",["Contact","FAQ","Privacy","Terms"]]].map(([t,ls]) => (
            <div key={t}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
                {ls.map(l => <a key={l} href="#" style={{ fontSize:"13px", fontWeight:300, color:"rgba(4,17,6,.5)", textDecoration:"none", transition:"color .2s" }} onMouseEnter={e=>e.target.style.color=G.ink} onMouseLeave={e=>e.target.style.color="rgba(4,17,6,.5)"}>{l}</a>)}
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <>
      ...
      <div style={{ minHeight:"100vh", background:G.bg }}>
        ...
        <section className="section-pad" style={{ maxWidth:1280, margin:"0 auto", padding:"72px 48px 0" }}>
          <div className="grid-contact" style={{ display:"grid", gridTemplateColumns:"1.25fr 1fr", gap:52, alignItems:"start" }}>
            <ContactForm />
            <ContactInfo />
          </div>
        </section>
